...
class SubmissionsModel extends Model
{
    ...
    public function get_submissions($assignment_id){
        #$db = db_connect();
        $query = $this->db->query("SELECT username, f_filename_internal, f_filename_original, f_created_at FROM submissions JOIN users ON users.id=submissions.s_user JOIN file_submission ON file_submission.submission = submissions.s_id JOIN files ON files.f_id = file_submission.file WHERE (s_id IN (SELECT MAX(s_id) as s_id FROM submissions WHERE (`s_assignment` = $assignment_id) GROUP BY `s_user`, `s_assignment`))");#->result_array();
